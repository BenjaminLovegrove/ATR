...
/////// Normals:
                ...
                float2 node_3797 = i.uv0;
                float2 node_1412 = (node_3797.rg+node_3798.g*float2(0,0.08));
                float3 node_1400 = tex2D(_Normal,TRANSFORM_TEX(node_1412, _Normal));
...
////// Emissive:
                float2 node_1977 = (i.uv0.rg+node_3798.g*float2(0.03,0.2));
                float3 emissive = (saturate(( _IllumValue > 0.5 ? (1.0-(1.0-2.0*(_IllumValue-0.5))*(1.0-(((1.0 - pow(node_1400.r,100.0))*pow(tex2D(_IllumAlphaMap,TRANSFORM_TEX(node_1977, _IllumAlphaMap)).r,3.0))*6.0))) : (2.0*_IllumValue*(((1.0 - pow(node_1400.r,100.0))*pow(tex2D(_IllumAlphaMap,TRANSFORM_TEX(node_1977, _IllumAlphaMap)).r,3.0))*6.0)) ))*_IllumColor.rgb);
///////// Gloss:
                float gloss = _GlossValue;
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                float2 node_12 = (node_3797.rg+node_3798.g*float2(0.025,0.05));
                ...
                finalColor += specular;
                finalColor += emissive;
                float2 node_3774 = (i.uv0.rg+node_3798.g*float2(0.03,1));
/// Final Color:
                return fixed4(finalColor,(tex2D(_AlphaTexturevertcolmult,TRANSFORM_TEX(node_3774, _AlphaTexturevertcolmult)).rgb*i.vertexColor.g*_AlphaAmount));
            }
            ENDCG
        }
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
/////// Normals:
                float4 node_3800 = _Time + _TimeEditor;
                float2 node_3799 = i.uv0;
                float2 node_1412 = (node_3799.rg+node_3800.g*float2(0,0.08));
                float4 node_1400 = tex2D(_Normal,TRANSFORM_TEX(node_1412, _Normal));
                float3 normalLocal = node_1400.rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float2 node_12 = (node_3799.rg+node_3800.g*float2(0.025,0.05));
                float3 node_9 = lerp(float3(i.uv0.rg,0.0),tex2D(_DistortAlphaMapoffset,TRANSFORM_TEX(node_12, _DistortAlphaMapoffset)).rgb,_Distortion);
                float4 node_2 = tex2D(_DistortDiffuseRGB,TRANSFORM_TEX(node_9, _DistortDiffuseRGB));
                float3 specularColor = float3(node_2.r,node_2.r,node_2.r);
                ...
                finalColor += diffuseLight * (_DiffColor.rgb*node_2.rgb*_DiffValue);
                finalColor += specular;
                float2 node_3774 = (i.uv0.rg+node_3800.g*float2(0.03,1));
/// Final Color:
                return fixed4(finalColor * (tex2D(_AlphaTexturevertcolmult,TRANSFORM_TEX(node_3774, _AlphaTexturevertcolmult)).rgb*i.vertexColor.g*_AlphaAmount),0);
            }
            ENDCG
        }
    }
